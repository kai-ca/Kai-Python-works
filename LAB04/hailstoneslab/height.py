"""Calculates how high the hailstone numbers get for 
a given seed.

Usage: import height
Usage: from height import measure
Usage: from height import measures

>>> measure(5)
16
>>> measure(7)
52
>>> measure(1)
1
>>> measures(6)
[(1, 1), (2, 2), (3, 16), (4, 4), (5, 16), (6, 16)]

>>> measures(5)

"""

from hailstones import stones

def measure(seed):
    """ For a given seed, returns the biggest of the stones.

    """

    return max(stones(seed))



def measures(maxseed):
    """Returns a list of pairs (seed, measure) for 
all seeds from to maxseed.

    For example:
    >>> measures(6)
    [(1, 1), (2, 2), (3, 16), (4, 4), (5, 16), (6, 16)]

    
    """

    return [(seed, measure(seed)) for seed in range(1, maxseed+1)]


  

