""" To decide which card ranks higher than 
another cards we look in the lists RANKS 
and SUITS.
    ...
    The index method returns the index 
(position) of the first occurence in the list.

    >>> cities = ['SF', 'LA', 'Boston', 'NY', 
    >>> 'London', 'Paris'] 
    >>> cities.index('Boston')
    2

    Tip: Print out the test files in the tests 
directory in this lab.
            Use those to decide exactly what 
your functions are supposed to do.
...
def suit_first_tuple(card):
    """Returns a tuple (s, r) where s is the 
suit index and r is the rank index of the 
card.
    """
